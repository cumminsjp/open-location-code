...
CREATE FUNCTION olc.open_location_code (	
) RETURNS text AS $$

 var open_loc_code  =  function () {
  ...
  var fn = open_loc_code();

  plv8.elog(NOTICE,'fn.encode(47.365590, 8.524997)=' + fn.encode(47.365590, 8.524997));

  plv8.elog(NOTICE,'fn.encode(47.365590, 8.524997, 11)=' + fn.encode(47.365590, 8.524997, 11));

  var coord = fn.decode('8FVC9G8F+6XQ');
  plv8.elog(NOTICE,'Center is ' + coord.latitudeCenter + ',' + coord.longitudeCenter);

  var shortCode = fn.shorten('8FVC9G8F+6X', 47.5, 8.5);
  plv8.elog(NOTICE,'shortCode=' + shortCode);

  return open_loc_code(); 
...
$BODY$
 ...
 var f  =  function () {
  ...
  return f().encode(p_latitude,p_longitude,p_code_length);
...
$BODY$
 ...
 var f  =  function () {
  ...
  return JSON.stringify(f().decode(p_code));	
...
-- SELECT olc.decode('8FVC9G8F+6XQ'::text);
  ...
  DROP FUNCTION IF EXISTS olc.to_geometry(text);
...
$BODY$
  ...
  COST 100;
  COMMENT ON FUNCTION olc.st_aspluscode(geometry, integer) IS 'Converts a plus code (open location code) to a PostGIS geometry.';
