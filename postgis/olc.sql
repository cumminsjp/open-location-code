...
CREATE SCHEMA IF NOT EXISTS olc AUTHORIZATION postgres;

COMMENT ON SCHEMA olc IS 'Open Location Code: https://github.com/google/open-location-code';

DROP FUNCTION IF EXISTS olc.open_location_code ();

DROP FUNCTION IF EXISTS olc.encode(float,float,integer);

DROP TABLE IF EXISTS olc.js_modules;

create table olc.js_modules (
    name text primary key,
    source text
);

insert into olc.js_modules values
('pluscode', 'function pluscode () {
...
CREATE OR REPLACE FUNCTION olc.encode(
  ...
  RETURNS text AS
$BODY$
	//  load module pluscode from the table js_modules
	var res = plv8.execute("select source from olc.js_modules where name = 'pluscode'");
	
	eval(res[0].source);
	//  now the function pluscode() is defined


	return pluscode().encode(p_latitude,p_longitude,p_code_length);
$BODY$
  LANGUAGE plv8 VOLATILE
  COST 100;

  
DROP FUNCTION IF EXISTS olc.decode(character varying);

CREATE OR REPLACE FUNCTION olc.decode(
...
$BODY$
	...
	return JSON.stringify(pluscode().decode(p_code));
$BODY$
  LANGUAGE plv8 VOLATILE
  COST 100;


-- SELECT olc.decode('8FVC9G8F+6XQ'::text);


CREATE OR REPLACE FUNCTION olc.to_geometry(
    p_code text)
  RETURNS geometry AS
$BODY$
...
DECLARE
	...
	l_geometry geometry;
BEGIN
	l_codearea := (olc.decode(p_code)::jsonb);

	RAISE NOTICE 'l_codearea=%',l_codearea;

	l_geometry :=
		-- geometry ST_MakeEnvelope(double precision xmin, double precision ymin, double precision xmax, double precision ymax, integer srid=unknown);
		-- st_asewkt(ST_MakeEnvelope((code_area ->> 'longitudeLo')::float, (code_area ->> 'latitudeLo')::float, (code_area ->> 'longitudeHi')::float, (code_area ->> 'latitudeHi')::float, 4326))
		ST_SetSRID(ST_MakePoint (
					(l_codearea ->> 'longitudeCenter')::float -- x coord
					,(l_codearea ->> 'latitudeCenter')::float -- y coord
					)
					,4326); -- as code_area_center_point		
...
$BODY$
  ...
  COST 100;
  COMMENT ON FUNCTION olc.to_geometry(text) IS 'Converts a plus code (open location code) to a PostGIS geometry.';


CREATE OR REPLACE FUNCTION olc.to_envelope(
    p_code text)
  RETURNS geometry AS
$BODY$
...
BEGIN
	...
	l_geometry :=
		-- geometry ST_MakeEnvelope(double precision xmin, double precision ymin, double precision xmax, double precision ymax, integer srid=unknown);
		ST_MakeEnvelope((l_codearea ->> 'longitudeLo')::float, (l_codearea ->> 'latitudeLo')::float, (l_codearea ->> 'longitudeHi')::float, (l_codearea ->> 'latitudeHi')::float, 4326)
		;
...
$BODY$
  ...
  COMMENT ON FUNCTION olc.to_envelope(text) IS 'Converts a plus code (open location code) to a PostGIS  envelope geometry.';
  
  -- SELECT st_asewkt(olc.to_envelope('8FVC9G8F+6XQ'))

 
  
-- SELECT ST_AsText(olc.to_geometry('8FVC9G8F+6XQ'));




CREATE OR REPLACE FUNCTION olc.ST_AsPlusCode(
    p_geo geometry
    ,p_code_length integer DEFAULT 10)
  RETURNS text AS
$BODY$
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
COMMENT ON FUNCTION olc.st_aspluscode(geometry, integer) IS 'Converts a geometry to plus code (open location code).';
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
  COMMENT ON FUNCTION olc.st_aspluscode(geography, integer) IS 'Converts a geography to plus code (open location code).';
